...
REPO="https://github.com/maksymsherman/force_rg.git"
INSTALL_DIR="${HOME}/.local/bin"
BINARY_NAME="enforce-rg-command"
CHECK_BINARY_HASH=0
OVERWRITE_BINARY=0
DRY_RUN=0
...
info "Cloning force_rg..."
git clone --depth 1 "$REPO" "$TMPDIR/force_rg" 2>/dev/null

info "Building..."
(cd "$TMPDIR/force_rg" && cargo build --release --quiet)

# --- install binary ---

mkdir -p "$INSTALL_DIR"
SOURCE_BINARY="$TMPDIR/force_rg/target/release/$BINARY_NAME"
